...
CHECKBOX_CAPTCHA_PROMPT = '''这是一张验证码弹窗的截图，截图尺寸为 {image_width}x{image_height} 像素。
坐标原点 (0,0) 在截图左上角，x 向右增大，y 向下增大。

弹窗中有一个"进行人机身份验证"的复选框（一个方形勾选框，通常在提示文字左侧）。
任务：找到这个复选框的中心点，返回应该点击的像素坐标。

要求：
- 返回复选框方框的正中心坐标，不要返回文字或其他区域
