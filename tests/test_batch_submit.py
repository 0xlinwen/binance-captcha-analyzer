...
from binance_cloud.batch_submit import submit_batches


class BatchSubmitTests(unittest.TestCase):
    @patch("binance_cloud.batch_submit.time.sleep")
    @patch("binance_cloud.batch_submit.requests.get")
    @patch("binance_cloud.batch_submit.requests.post")
    @patch("binance_cloud.batch_submit.load_accounts", return_value=[(f"a{i}@x.com", "pw") for i in range(5)])
    ...
    @patch("binance_cloud.batch_submit.time.sleep")
    @patch("binance_cloud.batch_submit.time.monotonic", side_effect=[0, 2])
    @patch("binance_cloud.batch_submit.requests.get")
    @patch("binance_cloud.batch_submit.requests.post")
    @patch("binance_cloud.batch_submit.load_accounts", return_value=[("a@x.com", "pw")])
